import React from 'react';

export interface TextareaProps extends React.TextareaHTMLAttributes<HTMLTextAreaElement> {
  label?: string;
  error?: string;
  helperText?: string;
}

export const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ label, error, helperText, className = '', ...props }, ref) => {
    return (
      <div className="w-full">
        {label && (
          <label className="block text-sm font-medium text-charcoal mb-1.5">
            {label}
            {props.required && <span className="text-alert-red ml-1">*</span>}
          </label>
        )}
        <textarea
          ref={ref}
          className={`w-full px-4 py-2 border rounded-md text-charcoal bg-white focus:outline-none focus:ring-2 focus:ring-mint-green focus:border-transparent transition-colors min-h-[100px] ${
            error ? 'border-alert-red' : 'border-light-gray'
          } ${className}`}
          {...props}
        />
        {error && (
          <p className="mt-1 text-sm text-alert-red">{error}</p>
        )}
        {helperText && !error && (
          <p className="mt-1 text-sm text-cool-gray">{helperText}</p>
        )}
      </div>
    );
  }
);

Textarea.displayName = 'Textarea';
